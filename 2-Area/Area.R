...
Area = function(source_folder){
  ...
  if(CheckFolder(source_folder,i)==1){
      ...
      #plot(fishnet)
      fish <- raster::intersect(tmp2,fishnet)
      ...
      if(is.null(tname)){fileout <- paste0(getwd(), "/Schema/", "5m_Area", tname,"_", rname, ".txt") }else{fileout <- paste0(getwd(), "/Schema/", "5m_Area_", tname,"_", rname, ".txt")}
